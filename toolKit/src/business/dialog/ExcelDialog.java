...
public class ExcelDialog extends TitleAreaDialog {
	...
	private ExcelModel jsp = null;
	/**
	 * Create the dialog
	 * @param parentShell
	 */
	public ExcelDialog(Shell parentShell,ExcelModel jsp) {
		super(parentShell);
		this.jsp = jsp;
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		try{
			...
			button.addMouseListener(new MouseAdapter() {
				public void mouseDown(final MouseEvent e) {
					try {
						...
						IFile file = OpenUtil.openSearch(jsp.getFile().getProject(), "src/ui_resource/excel", "xls");
						if(file!=null){
							File f = file.getLocation().toFile();
							String filePath = f.getAbsolutePath();
							path.setText("/"+filePath.replace(rootPath+"\\src\\ui_resource\\excel\\", "").replace("\\", "/"));
						}
					} catch (Exception e1) {
						e1.printStackTrace();
					}
			...
			returnType.add("基于模版导出");
			returnType.add("基于方法导出");
			returnType.select(returnTypeIndx);
			returnType.setBounds(75, 52, 122, 25);
			...
			label_2.setText("导出方式：");
			label_2.setBounds(10, 55, 78, 17);
			...
			label_3.setText("导出excel文件名:");
			label_3.setBounds(203, 24, 95, 17);

			text = new Text(composite, SWT.BORDER);
			text.setBounds(304, 21, 471, 25);
			text.setText(jsp.getFileName());
		}catch(Exception e){
			e.printStackTrace();
		}
		//
