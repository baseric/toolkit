...
import business.editor.menuaction.OpenFileAction;
...
public class ActionEditor extends GraphicalEditorWithFlyoutPalette {
	...
	protected PaletteRoot getPaletteRoot() {
		...
		try {
			...
			BufferedInputStream in =(BufferedInputStream) Activator.getInputStreamByName("/action.xml");
	...
	protected void createActions() {
		...
		IAction[] actions = new IAction[]{
				...
				new OpenFileAction(this,file,parent),
				new CreateCodeAction(this,file,parent,"action.ftl"),
	...
	public void doSave(IProgressMonitor monitor) {
		try {
			...
			if(!"N".equals(reader.getPropertyValue("isComplierOnSave"))){
				code.createAction(parent,file.getLocation().toFile(),file.getProject(),"action.ftl");
			}
		} catch (Exception ioe) {
			if(!(ioe instanceof ComplierException)){
				Log.write("", ioe);
			}
		}
	}
	...
	/*class OutlinePage extends ContentOutlinePage {
		...
		public void createControl(Composite parent) {
			...
			ContextMenuProvider menuProvider = new ViewEditorContextMenuProvider(getViewer(), getActionRegistry());
